...
<template>
  ...
  <div class="container mx-auto px-4 py-6 min-h-screen bg-gray-100 pt-16 pb-20">
    ...
    <div v-else-if="orders.length === 0" class="text-center text-[var(--espace-gris)]">
    ...
    <div v-else class="space-y-4">
      <div v-for="order in orders" :key="order.id" class="bg-[var(--espace-blanc)] rounded-lg shadow-md p-4">
        <div class="flex justify-between items-center mb-2">
          ...
          >
            {{ order.status === 'pending' ? 'En attente' : order.status === 'confirmed' ? 'Confirmée' : order.status === 'shipped' ? 'Expédiée' : order.status === 'delivered' ? 'Livrée' : 'Annulée' }}
          </span>
        </div>
        <div v-for="item in order.items" :key="item.produit_id" class="flex items-center gap-4 border-b py-2">
          <img
            :src="item.produit.photo_url || 'https://via.placeholder.com/50'"
            :alt="`Image de ${item.produit.nom}`"
